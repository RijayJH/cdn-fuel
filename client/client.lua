...
RegisterNetEvent('cdn-fuel:jerrycan:refuelmenu', function(itemData)
	...
	if holdingnozzle then
		local fulltank
		lib.callback('cdn_fuel:getfuel', false, function(fuel)
			if fuel == Config.JerryCanCap then fulltank = true
	...
	lib.callback('cdn_fuel:getfuel', false, function(fuel)
		...
		if refuel then
			...
			function() -- Play When Done
				...
				TriggerServerEvent('cdn-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, 'jerrycan')
...
RegisterNetEvent('cdn-fuel:jerrycan:refueljerrycan', function(data)
	local itemData = data.itemData
	local JerryCanMaxRefuel
	local jerrycanfuelamount
	local refuel
	lib.callback('cdn_fuel:getfuel', false, function(fuel)
		JerryCanMaxRefuel = (50 - fuel)
		jerrycanfuelamount = fuel
		print(fuel)
		refuel = exports['qb-input']:ShowInput({
		...
		if refuel then
			...
			function() -- Play When Done
				...
				local syphonData = data.itemData
				local srcPlayerData = QBCore.Functions.GetPlayerData()
				TriggerServerEvent('cdn-fuel:info', "add", tonumber(refuel.amount), srcPlayerData, "jerrycan")
...
RegisterNetEvent('cdn-syphoning:syphon:menu', function(itemData)
	lib.callback('cdn_fuel:getsyphon', false, function(fuel)
...
RegisterNetEvent('cdn-syphoning:syphon', function(data)
	lib.callback('cdn_fuel:getsyphon', false, function(fuel)
		...
		if HasSyphon then
			local currentsyphonamount = fuel
