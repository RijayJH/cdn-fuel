...
RegisterNetEvent("cdn-fuel:server:purchase:jerrycan", function(purchasetype)
	...
	local info = {cdn_fuel = Config.JerryCanGas,}
	if exports.ox_inventory:CanCarryItem(src, 'jerrycan', 1, info) then -- Dont remove money if AddItem() not possible!
...
RegisterNetEvent('cdn-fuel:info', function(type, amount, srcPlayerData, itemdata)
	...
	local search = exports.ox_inventory:Search(source, 1, itemdata)
    ...
    if type == "add" then
        search.metadata.cdn_fuel = search.metadata.cdn_fuel + amount
        exports.ox_inventory:SetMetadata(source, search.slot, search.metadata)
